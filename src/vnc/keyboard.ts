/**
 * Keysym helpers — ported 1:1 from mcp-vnc (src/vnc/keyboard.ts).
 */
...
const KEY_MAP: { [key: string]: number } = {
	...
	// Lock and misc keys (previously fell through to charCodeAt(0))
	CapsLock: 0xffe5, capslock: 0xffe5, Caps_Lock: 0xffe5,
	NumLock: 0xff7f, numlock: 0xff7f, Num_Lock: 0xff7f,
	ScrollLock: 0xff14, scrolllock: 0xff14, Scroll_Lock: 0xff14,
	PrintScreen: 0xff61, printscreen: 0xff61,
	...
	KP_Enter: 0xff8d, kpenter: 0xff8d,
	KP_Add: 0xffab, KP_Subtract: 0xffad, KP_Multiply: 0xffaa, KP_Divide: 0xffaf,
	KP_Decimal: 0xffae,
...
/**
 * Resolve a key name to its X keysym.
 *
 * :param key: Key name (e.g. "a", "Return", "F5", "CapsLock") or a single character
 * :return: The keysym value
 * :raises Error: If a multi-character key name is not in the key map (prevents
 *   silently sending the wrong key, e.g. "CapsLock" mapping to "C")
 */
export function getKeysym(key: string): number {
	if (key.length > 1 && !(key in KEY_MAP)) {
		throw new Error(`Unknown key: ${key}`);
	}
	return KEY_MAP[key] ?? key.charCodeAt(0);
}
